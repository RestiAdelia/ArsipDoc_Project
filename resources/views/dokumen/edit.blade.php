        ...
        <form action="{{ route('dokumen.update', $data->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="px-8 py-7 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-[11px] uppercase tracking-[0.2em] text-slate-400 font-medium mb-2">Nomor Dokumen</label>
                    ...
                    <div>
                        <label class="block text-[11px] uppercase tracking-[0.2em] text-slate-400 font-medium mb-2">Tanggal Dokumen</label>
                ...
                <!-- Nama Dokumen -->
                <div>
                    <label class="block text-[11px] uppercase tracking-[0.2em] text-slate-400 font-medium mb-2">Nama Dokumen</label>
                ...
                <!-- Jenis / Kategori -->
                <div>
                    <label class="block text-[11px] uppercase tracking-[0.2em] text-slate-400 font-medium mb-2">Jenis Dokumen</label>
                    <select name="kategori_id"
                            class="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none focus:border-slate-400 focus:bg-white transition-colors duration-200">
                        <option value="">— Pilih Kategori —</option>
                        @foreach($kategori as $k)
                            <option value="{{ $k->id }}" {{ old('kategori_id', $data->kategori_id) == $k->id ? 'selected' : '' }}>
                                {{ $k->nama_kategori }}
                            </option>
                        @endforeach
                    </select>
                ...
                <!-- Keterangan -->
                <div>
                    <label class="block text-[11px] uppercase tracking-[0.2em] text-slate-400 font-medium mb-2">Keterangan</label>
                ...
                <!-- Upload File -->
                <div class="border border-dashed border-stone-300 bg-stone-50/60 rounded-xl px-5 py-4 space-y-2">
                    <label class="block text-[11px] uppercase tracking-[0.2em] text-slate-400 font-medium">
                        Ganti File
                        <span class="normal-case tracking-normal text-slate-300 ml-1">(Opsional)</span>
                    </label>

                    @if($data->file_dokumen)
                        <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-white border border-stone-200 rounded-lg">
                            ...
                            <a href="{{ asset('storage/dokumen/'.$data->file_dokumen) }}" target="_blank"
                               class="text-xs text-slate-600 hover:text-slate-900 underline underline-offset-2 transition-colors">
                                {{ $data->file_dokumen }}
                            </a>
                        </div>
                    @endif
            ...
            <!-- Footer Aksi -->
            <div class="px-8 py-5 bg-stone-50 border-t border-stone-100 flex items-center justify-end gap-3">
                <a href="{{ route('dokumen.index') }}"
